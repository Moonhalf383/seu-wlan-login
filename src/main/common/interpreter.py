...
class Interpreter:
    def __init__(
        self, name: str = "System", description: str = "", plugin_dir: str = ""
    ):
        """
        ...
        # 如果指定了插件目录，则自动扫描并加载
        if plugin_dir:
            print(f"[log]Plugin dir found: {os.path.basename(plugin_dir)}[log]")
            print("[log]Dir existence: " + str(os.path.exists(plugin_dir)) + "[/log]")
                ...
                if spec and spec.loader:
                    ...
                    for member_name, member_obj in inspect.getmembers(module):
                        if (
                            ...
                            and member_obj is not Interpreter
                        ):  # 排除基类本身
                            # 实例化子解释器
                            ...
                            try:
                                ...
                                print(
                                    f"[error]Error instantiating {member_name}: {e}[/error]"
                                )
