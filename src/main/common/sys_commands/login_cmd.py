...
def login():
    try:
        ...
        try:
            ...
            return False
        printer("[log]Dr.COM状态检查接口连接成功...[/log]")

        if r.status_code != 200:
            render_error("错误：连接失败。[请求失败]")
            return False
        status = json.loads(pattern.findall(r.text)[0])
        printer("[log]状态请求成功...[/log]")

        if status["result"] == 1:
            printer("[warning]你已经登录了 seu-wlan。[/warning]")
            return False
        ...
        try:
            ...
            with open(config_file, "w") as f:
                ...
                return False
        printer("[log]正在读取配置文件...[/log]")
        if config["username"] == "" or config["password"] == "":
            printer(
                "[error]错误：请在配置文件 config.json 中填写用户名及密码。[/error]"
            )
            return False

        login_url = (
            "https://w.seu.edu.cn:801/eportal/?c=Portal&a=login&callback=dr1003&login_method=1&user_account=%2C0%2C"
            + config["username"]
            + "&user_password="
            + config["password"]
            + "&wlan_user_ip="
        ...
        try:
            ...
            render_error("错误：连接失败。[系统错误]")
            return False
        printer("[log]网络认证入口连接成功...[/log]")

        if r.status_code != 200:
            render_error("错误：连接失败。[请求失败]")
            return False
        login = json.loads(pattern.findall(r.text)[0])
        printer("[log]网络认证入口请求成功...[/log]")

        if login["result"] != "1":
            message = base64.b64decode(login["msg"]).decode()
            if message == "ldap auth error":
                printer("[error]错误：用户名或密码错误。[/error]")
            elif message == "userid error1":
                printer("[error]错误：用户名不存在。[/error]")
            elif message == "userid error2":
                printer("[error]错误：密码错误。[/error]")
            else:
                printer("[error]错误：登录失败。[/error]")
            return False

        printer("[warning]登录成功。[/warning]")
        return True

    except Exception as e:
        printer(f"[error]{e}[/error]")
        return False
